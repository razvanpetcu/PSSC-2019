...
namespace MVC_CORE.Repository
{
    public class Repository_clienti
    {
        ...
        public static bool First(Model_clienti m)
        {
            ...
            if (Verificare.ok)
            {
                ...
                using (MySqlConnection conn = GetConnection())
                {
                    ...
                    Repository_camere.SUPER_actualizare_camere();//aici o sa fie metoda SUPER_actualizare_camere
                    Actualizare_lista();
        ...
        public static string Update(Model_clienti m,string id,Tip_camera tip_vechi)
        {
            ...
            if (m.fail == null)
            {
                ...
                using (MySqlConnection conn = GetConnection())
                {
                    conn.Open();
                    string query;
                    m.id_camera = Cautare_camera_rezervata(tip_vechi);
                    query = "UPDATE tabel_clienti SET Nume='" + m.Nume + "',Prenume='" + m.Prenume + "',Tip_camera='" + m.tip_camera.ToString() + "'" +
                        ...
                        ",Numar_Carte='" + m.numar_carte + "',Data_expirare='" + m.Data_expirarii + "',CCV='" + m.CCV + "',Mentiuni='" + m.Mentiuni + "' WHERE ID='" + id + "';";
        ...
        public static void Actualizare_stare_clienti_o_data_pe_zi()
        {
            Actualizare_lista();
            foreach (Model_clienti m in lista_clienti)
            {
                if (m.check_out == DateTime.Now)
                {
                    using (MySqlConnection conn = GetConnection())
                    {
                        conn.Open();
                        string query;
                        query = "UPDATE tabel_clienti SET Stare='Inactiv' WHERE ID='"+m.ID+"';";

                        MySqlCommand cmd = new MySqlCommand(query, conn);
                        MySqlDataReader myreader;

                        myreader = cmd.ExecuteReader();//EXECUTE QUERY
                        Repository_camere.Eliberare_o_camera(m.tip_camera);
                    }
                }
            }
        }
